...
// The inference logic below is ported verbatim from src/lib/navi-model.ts
// (a custom client-side TypeScript transformer) and wrapped in a Deno handler.
//
// Contract (matches the shape App.tsx produces internally):
//   POST  body: { message: string, history: Array<{role:'user'|'assistant', content:string}> }
//   resp:       { response: string }
//   OPTIONS preflight handled for CORS.
//
// Deno-native: no npm imports, no browser globals, pure TypeScript math.
