// navi-mini — Claude Haiku for Mini subscribers
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Subscription check — mini OR max can use mini
    const subRes = await sb(
      `navi_subscriptions?email=eq.${encodeURIComponent(userEmail)}&status=eq.active&tier=in.(mini,max)&select=tier&limit=1`,
      { method: "GET" }
    );
    ...
    try {
      const rpcRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/navi_add_usage`, {
        ...
        body: JSON.stringify({ p_email: userEmail, p_tier: "mini", p_month_key: mk, p_cost: cost }),
      });
