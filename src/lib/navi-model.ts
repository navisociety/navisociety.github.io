...
// v13: Deterministic skills + conversational intelligence + practical life.
//      Skills: real arithmetic (25 × 17, 15% of 200, square roots, powers),
//      unit conversion (km/miles, kg/lbs, °C/°F, litres/gallons), and live
//      date/time answers — computed exactly, never retrieved. Conversation:
//      follow-up blending (a bare "why?" inherits the previous message as
//      retrieval context) and anti-repetition (never the same reply twice in
//      a row). Knowledge: 16 new practical-life nodes — CV writing, job
//      interviews & hunting, scams & phishing, passwords/privacy, NSFAS &
//      bursaries, matric, study skills, SARS, UIF, CIPC business registration,
//      driver's licence/K53, first aid + SA emergency numbers, SA capitals &
//      provinces, world geography (329 nodes total).
...
const KNOWLEDGE: KNode[] = [
  ...
  // ── Creative writing (v40) ─────────────────────────────────────────────────
  {
    ...
    responses: [
      'I love to write! Type /write followed by any prompt — like /write a poem about hope, /write a rap about the grind, /write 3 captions about the gym, or /write a letter to my future self. I can write stories, poems, songs, raps, prayers, letters, speeches, quotes, affirmations, captions, congratulations, condolences, apologies, thank-yous, and motivational pieces. Captions, quotes, and affirmations come up to six at once, and in funny or formal too — /write a formal quote about discipline. You can also just ask — "write me a prayer about strength" works too.',
    ],
    priority: 2,
  },
...
// ── Deterministic skills (v13): math, unit conversion, date/time ─────────────
// Exact answers computed before knowledge retrieval, so NAVI never guesses at
// arithmetic, conversions, or the calendar. Mirrored in
// supabase/functions/navi-chat/skills.ts (the server copy answers in South
// Africa time; this client copy uses the device's local time).
...
function tryUnits(message: string): string | null {
  const m = message
    .toLowerCase()
...
function tryDateTime(message: string, tz?: string): string | null {
...
function trySkills(message: string): string | null {
